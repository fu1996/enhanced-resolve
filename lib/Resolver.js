...
class Resolver {
	...
	static createStackEntry(hook, request) {
		console.log("createStackEntry", hook, request);
		const {
			descriptionFileRoot,
			descriptionFileData,
			descriptionFilePath,
		...
		stackPaths.push({
			当前调用的hook为: {
				name: hook.name,
				该hook绑定的plugin: hook.taps
			},
			上个hook结束的结果或传递给当前hook的参数: req
		});
		if (hook.name === "resolved") {
			fs.writeFileSync(
				path.join(context.fileDir, `${context.fileName}.json`),
				JSON.stringify(stackPaths, null, 2),
				{
					encoding: "utf-8"
				}
			);
	...
	doResolve(hook, request, message, resolveContext, callback) {
		...
		const stackEntry = Resolver.createStackEntry(hook, request, message);
